    ...
    LockAndMsgSender
{
    ...
    function dispatch(bytes1 commandType, bytes calldata inputs) internal returns (bool success, bytes memory output) {
        ...
        if (command < Commands.FOURTH_IF_BOUNDARY) {
            ...
            if (command == Commands.EXECUTE_SUB_PLAN) {
            ...
            } else if (command == Commands.V4_BIN_SWAP_EXACT_IN) {
                // equivalent: abi.decode(inputs, (V4BinExactInputParams, address, bool, bool))
                address msgSender;
                assembly {
                ...
                V4ExactOutputParams memory params = abi.decode(inputs.toBytes(0), (V4ExactOutputParams));
                _v4BinSwapExactOutput(params, msgSender, true, true);
            } else if (command == Commands.V4_BIN_SWAP_EXACT_OUT) {
                // equivalent: abi.decode(inputs, (V4CLExactOutputParams, address, bool, bool))
                address msgSender;
                assembly {
                    msgSender := calldataload(add(inputs.offset, 0x20))
                }
                V4CLExactOutputParams memory params = abi.decode(inputs.toBytes(0), (V4CLExactOutputParams));
                _v4CLSwapExactOutput(params, msgSender, true, true);
            } else {
                // placeholder area for commands 0x2a-0x3f
                revert InvalidCommandType(command);
            }
        }
    }
