...
import {PoolModifyPositionTest} from "pancake-v4-core/test/pool-cl/helpers/PoolModifyPositionTest.sol";
...
contract QuoterTest is Test, Deployers {
        ...
        returns (PoolKey memory)
    {
        ...
        return PoolKey(Currency.wrap(address(tokenA)), Currency.wrap(address(tokenB)), IHooks(hookAddr), ICLPoolManager(address(manager)), 3000, bytes32(uint256(0x3c0000)));
    }
        ...
        returns (IQuoter.QuoteExactParams memory params)
    {
        ...
        for (uint256 i = 0; i < _tokenPath.length - 1; i++) {
            path[i] = PathKey(Currency.wrap(address(_tokenPath[i + 1])), 3000, 60, IHooks(address(0)), ICLPoolManager(manager), bytes(""));
        }
        ...
        returns (IQuoter.QuoteExactParams memory params)
    {
        ...
        for (uint256 i = _tokenPath.length - 1; i > 0; i--) {
            path[i - 1] = PathKey(Currency.wrap(address(_tokenPath[i - 1])), 3000, 60, IHooks(address(0)), ICLPoolManager(manager), bytes(""));
        }
